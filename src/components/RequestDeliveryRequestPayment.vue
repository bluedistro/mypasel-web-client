<template lang="html">
  <div>
    ...
    <div class="col-md-5 col-lg-5 col-12 payment-div">
        ...
        <div class="row v-payment-row">
            <div class="col-md-4 col-4">
               <input type="radio" name="paymentOption" id="cash"
                      checked class="input-hidden" value="cash"
            ...
            <div class="col-md-4 col-4">
               <input type="radio" name="paymentOption" id="momo"
                      class="input-hidden" value="mobileMoney" :disabled="disableMobileMoneyOption"
                      v-model="paymentOption">
               <label for="momo">
                 Mobile Money
            ...
            <div class="col-md-4 col-4">
               <input type="radio" name="paymentOption" id="prepaid"
                      class="input-hidden" value="prepaid" :disabled="disablePrepaidOption"
                      v-model="paymentOption">
               <label for="prepaid">
                 Prepaid
              ...
              <div class="row">
                  ...
                  <div class="col-md-4 col-sm-4 col-4">
                    <label class="switch" id="toggleRadio">
                      <input type="checkbox" :disabled="disablePromoCodeFeature"
                             v-model="toggleState" @click="changeToggleState">
                      <span class="slider"></span>
                    </label>
                    <b-tooltip v-show="disablePromoCodeFeature"
                               target="toggleRadio" placement="bottom"
                               title="feature is disabled"></b-tooltip>
                  </div>
              </div>
        ...
        <div class="col-md-12">
            <button type="button"
                    @click="confirmOrder" v-promise-btn
    ...
    <!-- Unable to book -->
    <div>
      <b-modal no-close-on-backdrop hide-header-close no-close-on-esc ref="booking-error-modal"
               size="sm" id="booking-error-modal" v-model="unsuccessfulBookingModal" title="Booking Status">
        <p class="booking-error-modal-text">Sorry, we were unable to complete your transaction booking.</p>
        <template slot="modal-footer" slot-scope="{ok, cancel}">
          <b-button size="sm" variant="warning" @click="bookingErrorModalHide" >Start new request</b-button>
          <b-button size="sm" variant="outline-dark" @click="bookingErrorModalTry" >Try Again</b-button>
        </template>
      </b-modal>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    checkPromoCodeValidity(evt){
      ...
      this.$store.dispatch('validatePromoCode', payload)
                 .then((resp) => {
    ...
    confirmOrder(evt){
      ...
      return this.$store.dispatch('confirmOrder', payload)
                 .then((resp) => {
