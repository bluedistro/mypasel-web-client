  ...
  actions: {
    ...
    login({commit}, user){
        ...
        .then((resp) => {
          ...
          VueCookie.set(cookeys.USER_DATA_KEY, JSON.stringify(user), {expires: cookeys.cookie_expire})
...
// and ugly hack to prevent user from going back to the courier found page
    ...
    getPricing({commit}, requestPayload){
      ...
      return new Promise((resolve, reject) => {
        ...
        if(requestPayload.dropOffData.length == 1){
            ...
            VueCookie.set(cookeys.REQUEST_DELIVERY_PAYLOAD_KEY, JSON.stringify(requestPayload), {expires: cookeys.cookie_expire});
            axios.get(path, { params: params})
                 .then((resp) => {
                   ...
                   VueCookie.set(cookeys.PRICING_KEY, JSON.stringify(pricing), {expires: cookeys.cookie_expire})
                   resolve(resp)
                 })
                 .catch((error) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        axios.post(path, payload)
             .then((resp) => {
               ...
               VueCookie.set(cookeys.BOOKING_SUCCESS_PAYLOAD_KEY, JSON.stringify(bookingInfo), {expires: cookeys.cookie_expire})
    ...
    getOngoingTransactions({commit}, id){
      return new Promise((resolve, reject) => {
         ...
         axios.get(path)
              .then((resp) => {
                const data = resp.data
                VueCookie.set(cookeys.ONGOING_TRANSACTIONS_DATA_KEY, JSON.stringify(data), {expires: cookeys.cookie_expire})
    ...
    getTransactionsHistory({commit}, id){
      return new Promise((resolve, reject) => {
        const path = "https://api.desymal.com/user/"+String(167)+"/finishedtransactions"
        axios.get(path)
             .then((resp) => {
